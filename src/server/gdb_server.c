	...
	char const *packet, int packet_size)
{
	...
	if (current_thread_id_hexagon > 1 && (current_target_hexagon))
	{
		...
		if (temp_reg_num == 40)
		{
			...
			gdb_str_to_target(target, hexagon_sp, reg_list[reg_num - 11]);
			gdb_str_to_target(target, hexagon_fp, reg_list[reg_num - 10]);
		}
		else if (temp_reg_num == 29)
		{
			gdb_str_to_target(target, hexagon_sp, reg_list[reg_num]);
			gdb_str_to_target(target, hexagon_fp, reg_list[reg_num + 1]);
			gdb_str_to_target(target, hexagon_pc, reg_list[reg_num + 11]);
		}
		else if (temp_reg_num == 30)
		{
			gdb_str_to_target(target, hexagon_fp, reg_list[reg_num]);
			gdb_str_to_target(target, hexagon_sp, reg_list[reg_num - 1]);
